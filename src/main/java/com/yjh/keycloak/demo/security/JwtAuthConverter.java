...
public class JwtAuthConverter implements Converter<Jwt, AbstractAuthenticationToken> {
	...
	@Value("${keycloak.urls.userinfo}")
	private String userinfoUrl;
	
	
	public boolean getKeycloakUserInfo(String jwtToken)  {
		
		
		try {
			
			RestTemplate restTemplate = new RestTemplate();
			UriComponents uri = UriComponentsBuilder.fromHttpUrl(baseUrl+userinfoUrl).build();
		    ...
		    Object res = restTemplate.exchange(uri.toString()
		    									, HttpMethod.GET
		    									, request
		    									, new ParameterizedTypeReference<Object>(){}).getBody();   
		    log.info("keycloak response userInfo => {}" , res);
		    if(res != null) {
		    	return true;
		    }
		} catch (Exception e) {
    ...
    public AbstractAuthenticationToken convert(Jwt jwt)   {
		...
		//token 분석 전 keycloak과 통신하여 user정보가 있는지 확인.
		log.info("jwt.getTokenValue() == {}",jwt.getTokenValue()); 
		boolean flag = getKeycloakUserInfo(jwt.getTokenValue());
		if(!flag) {
			return null;
		}
